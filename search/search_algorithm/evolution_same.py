...
class EvolutionFinder:
	...
	def __init__(self, efficiency_predictor, accuracy_predictor,prob_map, **kwargs):
	...
	def random_valid_sample(self, constraint):
		constraint_w=constraint[0]
		constraint_a=constraint[1]
		while True:
			sample = self.arch_manager.random_sample_arch(constraint,self.prob_map)
		

			avg_w,avg_a=self.efficiency_predictor.get_efficiency(sample)
			if constraint_w-self.step<=avg_w <= constraint_w and constraint_a-self.step<=avg_a <= constraint_a:
				return sample,(avg_w,avg_a)
	...
	def mutate_sample(self, sample, constraint):
		...
		while True:
			...
			avg_w,avg_a=self.efficiency_predictor.get_efficiency(new_sample)
			if constraint_w-self.step<=avg_w <= constraint_w and constraint_a-self.step<=avg_a <= constraint_a:
				return new_sample,(avg_w,avg_a)
	...
	def crossover_sample(self, sample1, sample2, constraint):
		...
		while True:
			...
			if not isinstance(new_sample['w_bit_list'], list):
				new_sample['w_bit_list'] = new_sample['a_bit_list']=random.choice([sample1['w_bit_list'], sample2['w_bit_list']])
			else:
				for i in range(len(new_sample['w_bit_list'])):
					new_sample['w_bit_list'][i] = new_sample['a_bit_list'][i]=random.choice([sample1['w_bit_list'][i], sample2['w_bit_list'][i]])
	...
	def run_evolution_search(self, constraint, verbose=False, **kwargs):
		...
		with tqdm(total=self.max_time_budget, desc='Searching with constraint_w (%s) constraint_a (%s)' % (constraint[0],constraint[1]),
			...
			for i in range(self.max_time_budget):
				...
				for j in range(mutation_numbers):#变异数量
				...
				for j in range(self.population_size - mutation_numbers):#交叉数量
