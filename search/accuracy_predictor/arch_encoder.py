# arch manager: structure encode and sample
...
__all__ = ['MobileNetArchEncoder', 'ResNetArchEncoder', 'OQAEncoder']
...
class ResNetArchEncoder:
    ...
    def __init__(self, image_size_list=None, depth_list=None, expand_list=None, width_mult_list=None,
        ...
        self.image_size_list = [24, 32, 40, 48] if image_size_list is None else image_size_list
...
def sample_helper(bw, m):
    ...
    # return random.choices(keys)[0]# uniform
    return random.choices(keys, weights=probs)[0]  # probmap


class OQAEncoder:
    def __init__(self, bw_weights_list=None, bw_fm_list=None, module_nums=None):
        ...
        self.n_dim = 6
        # bw_weights
        self.bw_weights_info = dict(id2val=[], val2id=[], L=[], R=[])
        self._build_info_dict(target='bw_weights')
        # bw_fm
        self.bw_fm_info = dict(id2val=[], val2id=[], L=[], R=[])
        self._build_info_dict(target='bw_fm')

    def _build_info_dict(self, target):
        if target == 'bw_weights':
            target_dict = self.bw_weights_info
            choices = self.bw_weights_list
            ...
            for i in range(self.module_nums):
                ...
                target_dict['L'].append(self.n_dim)
                for bw in choices:
                    target_dict['val2id'][i][bw] = self.n_dim
                    target_dict['id2val'][i][self.n_dim] = bw
                    self.n_dim += 1  # 140=20*7
                target_dict['R'].append(self.n_dim)

        elif target == 'bw_fm':
            target_dict = self.bw_fm_info
            choices = self.bw_fm_list
    ...
    def arch2feature(self, arch_dict, channel_wise, w_sym, a_sym):
        w, a = arch_dict['w_bit_list'], arch_dict['a_bit_list']
        feature = np.zeros(self.n_dim)

        range_num = [i for i in range(self.module_nums)][1:-1]  # 第一层和最后一层不编码，因为是一致的
        # range_num=[i for i in range(self.module_nums)]

        for i in range_num:  #
            feature[self.bw_weights_info['val2id'][i][w[i]]] = 1
            feature[self.bw_fm_info['val2id'][i][a[i]]] = 1
    ...
    def random_sample_arch(self, constraint, prob_map):
        archs = {}
        con_id = 0
        for k in ['w_bit_list', 'a_bit_list']:
            temp_list = []
            for idx in sorted(list(prob_map[k].keys())):
                temp_list.append(sample_helper(constraint[con_id], prob_map[k][idx]))
            con_id += 1
            archs[k] = temp_list
        return archs
    ...
    def mutate_arch(self, arch_dict, mutate_prob):
        ...
        for i in range(len(arch_dict['w_bit_list'])):
            if random.random() < mutate_prob:
                arch_dict['w_bit_list'][i] = random.choice(self.bw_weights_list)

        for i in range(len(arch_dict['a_bit_list'])):
            if random.random() < mutate_prob:
                arch_dict['a_bit_list'][i] = random.choice(self.bw_fm_list)
